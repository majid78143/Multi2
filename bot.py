...
async def reactionrole(ctx, action: str = "list", message_id: str = None, emoji: str = None, role: discord.Role = None):
    if not has_manage_guild(ctx.author):
        return await ctx.send(embed=branded_embed(description="You don't have permission to do this."))
    data = get_guild_data(ctx.guild.id)
    action = action.lower()
...
async def maintenance_cmd(ctx, state: str = None):
    if not is_owner(ctx.author.id):
        return
    data = get_global_data()
    if state is None:
        ...
        data["maintenance"] = state.lower() in ("on", "enable", "true", "1")
    save_global_data(data)
    status = "🔴 ON" if data["maintenance"] else "🟢 OFF"
    await ctx.send(embed=branded_embed(description=f"Maintenance mode: **{status}**\nUsage: `!maintenance on` / `!maintenance off` / `!maintenance` (toggle)"))
...
async def on_ready():
    ...
    # Per-guild sync first — shows slash commands INSTANTLY in every server the bot is in
    for guild in bot.guilds:
        try:
            guild_synced = await bot.tree.sync(guild=guild)
            print(f"[Pixel] Slash synced in {guild.name}: {len(guild_synced)} commands")
        except Exception as e:
            print(f"[Pixel] Guild slash sync failed for {guild.name}: {e}")

    # Global sync too — ensures new servers get commands after ~1hr propagation
    try:
        synced = await bot.tree.sync()
        print(f"[Pixel] Global slash sync done: {len(synced)} commands")
    except Exception as e:
...
# Usage hints shown when a command is used without required arguments
USAGE_HINTS = {
    "ban": "ban @user [reason]",
    "kick": "kick @user [reason]",
    "mute": "mute @user [minutes] [reason]",
    "unmute": "unmute @user",
    ...
    "setwelcome": "setwelcome #channel [message]",
    "setleave": "setleave #channel [message]",
    "setautorole": "setautorole @role",
    "automod": "automod on|off",
    "setlogchannel": "setlogchannel #channel",
    "setmodlog": "setmodlog #channel",
    "setverification": "setverification @role",
    "setsuggestionchannel": "setsuggestionchannel #channel",
    "setsticky": "setsticky <message>",
    "togglecommand": "togglecommand <command_name> on|off",
    "pay": "pay @user <amount>",
    "buy": "buy <item_name>",
    "remind": "remind <time> <message>  e.g. 10m check email",
    "translate": "translate <lang> <text>  e.g. translate hi Hello",
    "giveaway": "giveaway <duration> <prize>  e.g. giveaway 1h Nitro",
    "ticket": "ticket [reason]",
    "reactionrole": "reactionrole add <msg_id> <emoji> @role  |  reactionrole remove <msg_id> <emoji>  |  reactionrole list",
    "rolemenu": "rolemenu <menu_id>",
    "maintenance": "maintenance [on|off]  (no args = toggle)",
    "broadcast": "broadcast <message>",
    "setstatus": "setstatus <text>",
    "forceleave": "forceleave <guild_id>",
    "blacklist": "blacklist <user_id>",
    "setbirthday": "setbirthday DD/MM",
    "suggest": "suggest <your suggestion>",
    "poll": "poll <question>",
}
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, (commands.MissingRequiredArgument, commands.BadArgument)):
        ...
        hint = USAGE_HINTS.get(cmd)
        if hint:
            msg = f"**Usage:** `{config.DEFAULT_PREFIX}{hint}`"
        else:
            msg = f"Invalid usage. Type `{config.DEFAULT_PREFIX}help` for all commands."
        return await ctx.send(embed=branded_embed(description=msg))
    print(f"[Pixel] Command error in {ctx.command}: {error}")
...
async def on_message(message):
    ...
    # Wrap everything in try/finally — process_commands MUST always run
    try:
        ...
        try:
            ...
            print(f"[Pixel] Auto-responder error: {_e}")

        # automod
        automod_blocked = False
        try:
            ...
            if automod.get("enabled"):
                ...
                if triggered_reason:
                    automod_blocked = True
                    try:
                        await message.delete()
                    except discord.Forbidden:
                        pass
                    await message.channel.send(
                        embed=branded_embed(description=f"{message.author.mention} your message was removed ({triggered_reason})."),
                        delete_after=5,
                    )
                    ...
                    if strikes[uid] >= threshold:
                        ...
                        try:
                            if action == "ban":
                                await message.author.ban(reason="Auto-mod strikes exceeded")
                            elif action == "kick":
                                await message.author.kick(reason="Auto-mod strikes exceeded")
                            else:
                                await message.author.timeout(
                                    datetime.timedelta(minutes=automod.get("strike_mute_minutes", 30)),
                                    reason="Auto-mod strikes exceeded",
                                )
                            strikes[uid] = 0
                            save_guild_data(message.guild.id, data)
                        except discord.Forbidden:
            ...
            print(f"[Pixel] Automod error: {_e}")

        if not automod_blocked:
            # leveling
            try:
                profile = data.setdefault("levels", {}).setdefault(str(message.author.id), {"xp": 0, "level": 0})
                ...
                if profile["xp"] >= needed:
                    ...
                    reward_role = None
                    reward_role_id = levelup.get("role_rewards", {}).get(str(profile["level"]))
                    if reward_role_id:
                    ...
                    sent = await _send_template_or_fallback(
                        target_channel, message.author, message.guild, "levelup", data,
                ...
                print(f"[Pixel] Leveling error: {_e}")
            ...
            try:
                ...
                print(f"[Pixel] Sticky error: {_e}")

    except Exception as _e:
        print(f"[Pixel] on_message error: {_e}")
    finally:
        # ALWAYS process commands — this is what makes prefix + slash work
