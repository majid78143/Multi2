...
def dashboard_guild(guild_id):
    ...
    <div class="container">
      ...
      <form method="POST" class="card">
        ...
        <textarea name="welcome_message">{{ data['welcome']['message'] }}</textarea>
        ...
        <textarea name="leave_message">{{ data['leave']['message'] }}</textarea>
      ...
      <div class="card">
        ...
        <form method="POST" action="/dashboard/{{ guild.id }}/action/announce">
          ...
          <select name="channel_id">{% for c in text_channels %}<option value="{{ c.id }}">#{{ c.name }}</option>{% endfor %}</select>
          <textarea name="message" placeholder="Message to send" style="margin-top:8px;"></textarea>
